...
def plot(map_data, plot_name):
  fig, ax = plt.subplots()
  im1 = ax.imshow(map_data, cmap='jet', aspect='auto')
  ax.set_title('{}'.format(plot_name))
  plt.colorbar(im1, ax=ax) 
  plt.show()

### test
ignition_probability_map = np.where(ignition_probability_map > 0, ignition_probability_map + 0.5, 0)
print(ignition_probability_map.min(), ignition_probability_map.max())

ignition_proba = ignition_probability_map.copy()
fuel = fuel_map.copy()
...
def fire_propagation(scenarios, fuel=fuel_map, ignition_proba=ignition_probability_map):
    ...
    for scenario in range(scenarios):
        ...
        fire = np.zeros((time_total, fuel_map.shape[0], fuel_map.shape[1]))
        fire[0] = ignition[0] + fuel  # 0 = no fuel, 1 = fuel, 2 = fire
        plot(fire[0], 'ingnition_{}'.format(scenario))
        ...
        # gif(fire, scenario)
        plot(fire[time_total], scenario)
        print(" --- {} scenario in {} seconds".format(scenarios, (round(time.time() - start_time, 2))))
        print("--- {}s seconds ---" % (time.time() - start_time))
    return fires

# start_time = time.time()
fires_list = fire_propagation(2)

final_fires_array = np.array(fires_list)
fire_impact = np.sum(final_fires_array)

plot(fire_impact, 'final_impact')